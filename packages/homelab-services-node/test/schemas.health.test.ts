...
describe("HealthResponseSchema", () => {
  describe("decode", () => {
    it.effect("should decode 'All services healthy' message", () =>
      Effect.gen(function*() {
        const result = yield* Schema.decode(Schemas.Health.HealthResponseSchema)("All services healthy")
    ...
    it.effect("should decode single unhealthy service", () =>
      Effect.gen(function*() {
        const result = yield* Schema.decode(Schemas.Health.HealthResponseSchema)("Jellyfin is Unreachable")
    ...
    it.effect("should decode multiple unhealthy services", () =>
      Effect.gen(function*() {
        const result = yield* Schema.decode(Schemas.Health.HealthResponseSchema)(
  ...
  describe("encode", () => {
    it.effect("should encode all healthy services", () =>
      Effect.gen(function*() {
        const result = yield* Schema.encode(Schemas.Health.HealthResponseSchema)({
          Jellyfin: "Healthy",
          Kanidm: "Healthy",
          "Step-CA": "Healthy",
          RADIUS: "Healthy",
  ...
  describe("round-trip encode/decode", () => {
    it.effect("should round-trip for any valid health status record", () =>
      Effect.gen(function*() {
        const arbitrary = Arbitrary.make(Schema.typeSchema(Schemas.Health.HealthResponseSchema))
            ...
            FastCheck.assert(
              FastCheck.asyncProperty(
                ...
                (healthStatus) =>
                  pipe(
                    healthStatus,
                    Schema.encode(Schemas.Health.HealthResponseSchema),
                    Effect.flatMap(Schema.decode(Schemas.Health.HealthResponseSchema)),
